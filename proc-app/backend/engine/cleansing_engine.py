...
_ENGINE_ONLY: list[dict] = [
    ...
    {"rule_id": "gold.po_type_derivation",                "note": "Gold V1 enrichment (KB-PART-5)"},
    {"rule_id": "gold.pac_detection_column",              "note": "Gold V1 enrichment (KB-PART-5)"},
    {"rule_id": "gold.emergency_detection",               "note": "Gold V1 enrichment (KB-PART-5)"},
    {"rule_id": "gold.capex_flag",                        "note": "Gold V1 enrichment (KB-PART-5)"},
    {"rule_id": "gold.approver_tier",                     "note": "Gold V1 enrichment (KB-PART-5)"},
...
_KB_PART_5_COVERAGE: dict[str, dict] = {
    ...
    "designation_lookup":                {"stat": "implemented", "fires_as": "gold.approver_tier",
                                            "note": "V1 seed via designation-tier-seed.yml; V2 adds per-engagement dictionary"},
    "tier_assignment":                   {"stat": "implemented", "fires_as": "gold.approver_tier",
                                            "note": "Maps designation → tier 1-5; DoA pillar uses gold approver_tier column"},
    # PO type tagging
    "explicit_po_type_field":            {"stat": "implemented", "fires_as": "gold.po_type_derivation",
                                            "note": "Gold enrichment: uses explicit po_type column when present"},
    "derive_po_type_from_category":      {"stat": "implemented", "fires_as": "gold.po_type_derivation",
                                            "note": "Gold enrichment: keyword scan on material_group_desc"},
    "pac_detection":                     {"stat": "implemented",
                                            "fires_as": "procurement.short_text_pac_detection + gold.pac_detection_column",
                                            "note": "Bronze rule fires + Gold materialises as is_pac column"},
    "emergency_detection":               {"stat": "implemented", "fires_as": "gold.emergency_detection",
                                            "note": "Gold enrichment: short_text keyword scan"},
